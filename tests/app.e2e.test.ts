...
describe('app e2e', { timeout: 60_000 }, () => {
  ...
  it('returns rendered html from /content', async () => {
    ...
    const response = await request(app).post('/content').auth(authUser, authPassword).send({ url: 'https://example.com' });
  ...
  it('returns png bytes from /screenshot', async () => {
    ...
    const response = await request(app)
      ...
      .parse((res, callback: (error: Error | null, body: Buffer) => void) => {
        ...
        res.on('error', (error: Error) => callback(error as Error, Buffer.alloc(0)));
      });
  ...
  it('returns 401 when auth is missing', async () => {
    const app = createApp(browserService as BrowserService);
    const response = await request(app).post('/content').send({ url: 'https://example.com' });
